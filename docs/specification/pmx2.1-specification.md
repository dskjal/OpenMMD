...
[URL](https://gist.githubusercontent.com/felixjones/f8a06bd48f9da9a4539f/raw/57b4491bd54540a2ecebbee81d61d6fd8c31b99d/pmx21.md)
[How MikuMikuDance Animation Works #1: Morphing and Skinning](https://qiita.com/dragonmeteor/items/0211166d55bb2eb7c07c)
...
The order in which materials are listed is the order in which they are rendered.
...
See [How MikuMikuDance Animation Works #1: Morphing and Skinning](https://qiita.com/dragonmeteor/items/0211166d55bb2eb7c07c) and [MMD's SDEF Skinning on Blender](https://gist.github.com/nagadomi/aa39745ae6716b50c2a60288b093d14b).

Spherical deform blending.

$p^{\mathrm{vm}}$ is vertex position (after morphing).

$\mathbf{M}$ is the matrix for linear blend skinning.

$$ \mathbf{M} = w_0 \mathbf{M}_{k_0} + w_1 \mathbf{M}_{k_1}$$

$$
\tilde{\mathbf{R}} = w_0\mathbf{R}_0 + w_1\mathbf{R}_1\\
\mathbf{C}_0 = \mathbf{C} + \dfrac{\mathbf{R}_0 - \tilde{\mathbf{R}}}{2}  \\
\mathbf{C}_1 = \mathbf{C} + \dfrac{\mathbf{R}_1 - \tilde{\mathbf{R}}}{2}   \\
\mathbf{p}^{\mathrm{fin}} = w_0 \mathbf{M}_{k_0}\mathbf{C}_0+ w_1 \mathbf{M}_{k_1}\mathbf{C}_1 + \mathbf{R}(\mathbf{p}^{\mathrm{vm}} -\mathbf{C})
$$

```
https://x.com/mqdl/status/893813138599981056

4x4の行列でボーン変形させると原理上どうしても潰れる。
じゃQuaternionで回転させようぜ！
となるとQDEFの様に膨らみ問題が出てしまうので、じゃ、回転中心を求めればよいんじゃ？　って言うのがSDEFのC値で、C値は動的に可変させる必要があるのでそれに使うのが二つのR値
```

Skinning with simple linear interpolation (4x4 matrices) causes joints to collapse (the "candy-wrapper effect"). Conversely, rotating with QDEF (Quaternions) leads to bulging around the joint areas. SDEF (Spherical Deformation) was designed to control this bulging by allowing the center of rotation to be manually adjusted.

The following is an explanation from ChatGPT 5.4:

"The concept of SDEF is quite straightforward: it assigns information to each vertex regarding 'which point this vertex should rotate around.' In the Spherical Blend Skinning described in the article, the vertex $p^{vm}$ is split into a 'rotation center `C`' and an 'offset from that center $p^{vm} - C$.' The `C` component is transformed using `M` (the linear blend of bone matrices), while the offset component is rotated by `R` (the bone rotation blended via quaternions). The final equation is 

$$p^{fin} = M C + R(p^{vm} - C)$$

SDEF builds on this by adding `R0` and `R1` . In the implementation discussed, `R0` and `R1` are first blended by bone weights to create $\tilde{R} = w_0 R_0 + w_1 R_1$, from which `C0` and `C1` are shifted to calculate the final position. When expanded, SDEF effectively becomes:

$$p^{fin} = M C + R(p^{vm} - C) + (w_0 w_1 / 2)(M_0 - M_1)(R_0 - R_1)$$

This takes the form of 'standard spherical blend skinning plus an additional correction term.' In other words, it is more appropriate to view `R0` and `R1` as 3D position parameters used to dynamically correct the rotation center `C` , rather than simply as 'two radii.' Since only the difference `R0 - R1` carries weight, it is easiest to think of it as defining 'in which direction and by how much the center should be offset.'"

In Spherical Blend Skinning, if you set C にto the vertex coordinates themselves, it reverts to simple linear interpolation. In SDEF, setting R0 = R1 causes the additional term to disappear, effectively turning it back into Spherical Blend Skinning.

Consider the case where R0 = R1 i.e. ($p^{\mathrm{fin}} = M C + R(p^{\mathrm{vm}} - C)$). In this case, the vertex position is represented by C + (p - C). In other words, the original vertex is represented by the vertex C and a vector from C. The vertex C is transformed using the standard skinning matrix. The vector from C can be controlled by the Bone 1 weight to determine which bone’s rotation affects it.

Assume Bone 1 is the upper arm and Bone 2 is the lower arm. When the Bone 1 weight is 1, the difference vector (p - C) is not affected by the lower arm’s rotation at all, thereby preventing joint sinking (C is affected by the lower arm).

By using $R_0$ and $R_1$, we can express $C$ as “$C +$ a vector difference” and control the amount of rotation applied by the Bone 1 weight.

It would have been more convenient if we could specify the offset vector directly, rather than having to define R0 and R1.
...
|index	|Bone index 2	|									|
|float	|Bone 1 weight	|Bone 2 weight = 1.0 - Bone 1 weight|
|vec3	|C				|Rotation center								|
|vec3	|R0				|The starting point of the vector that shifts the center of rotation|
|vec3	|R1				|The ending point of the vector that shifts the center of rotation|
...
|3			|Is visible				|Visible in the viewport and bone list				|2.0		|
|4			|Enabled				|???										|2.0		|
...
|11			|Local co-ordinate		|Local coordinate							|2.0		|
|12			|Physics after deform	|???										|2.0		|
|13			|External parent deform	|For example, child constraint in Blender    |2.0		|

OpenMMD note: when importing PMD, bones with `Type = 2` (IK bones) are normalized with both `Rotatable` and `Translatable` enabled in the internal PMX-like model data.
...
|Name		|Type	|Notes	|
|-----------|-------|-------|
|X vector	|vec3	|Local x direction	|
|Z vector	|vec3	|Local z direction	|

## Bone External Parent

|Name			|Type	|Notes	|
|---------------|-------|-------|
|Parent index	|index	|Exterenal parent bone index	|
...
|Loop count		|int	|CCD iteration count										|
|Limit radian	|float	|IK rotation limit in radian										|
